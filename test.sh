...
MESSAGES[zh_input_obfs_pwd]="混淆密码 (默认生成 UUID，用于 Salamander 混淆): "
...
MESSAGES[en_input_obfs_pwd]="Obfuscation password (default UUID generated, for Salamander obfuscation): "
...
install_hysteria() {
  ...
  while [[ $retry_count -lt $max_retries ]]; do
    ...
    if [[ "$version" < "v2.6.2" ]]; then
      log "Warning: Detected version $version is older than v2.6.2, forcing latest download / 警告：检测版本 $version 低于 v2.6.2，强制下载最新版" "$YELLOW"
    fi
...
create_config() {
  ...
  read -p "$(get_msg input_obfs_pwd)" obfs_password
...
generate_configs() {
  ...
  local sub_link="hysteria2://$password@$domain:$main_port/?insecure=1&obfs=salamander&obfs-password=$obfs_password&hopPorts=$port_range#[DIY_Hy2]$hostname"
